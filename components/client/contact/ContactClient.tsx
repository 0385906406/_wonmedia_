...
function BannerSection({ t }: { t: ContactT }) {
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: 220, overflow: 'hidden', display: 'flex', alignItems: 'center', color: '#fff' }}>
      <motion.div
      ...
      <div className="container won-contact-banner" style={{ position: 'relative', zIndex: 10, paddingBlock: '52px' }}>
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={VP} transition={{ duration: 0.7, delay: 0.1 }}
          ...
          <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.82)', fontFamily: 'var(--font-primary)' }}>{t.bannerSubtitle}</span>
        </motion.div>
        <motion.h1
